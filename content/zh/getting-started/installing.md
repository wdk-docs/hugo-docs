...
title: 安装 Hugo
linktitle: 安装 Hugo
description: 在Mac OS，Windows，Linux和OpenBSD的，FreeBSD的安装Hugo和任何机器上，其中Go编译工具链都可以运行。
date: 2016-11-01
...
- Type in the folder where `hugo.exe` was extracted, which is `C:\Hugo\bin` if you went by the instructions above. _The PATH entry should be the folder where Hugo lives and not the binary._ Press <kbd>Enter</kbd> when you're done typing.
